...
if( isset( $_GET[ 'Login' ] ) ) {
	...
	// Real anti-automation: lock this account out of THIS security level
	// for a while after too many failed attempts in a row. A fixed sleep()
	// alone can't stop scripted brute-forcing since it just slows the same
	// guaranteed-to-eventually-succeed loop down, it doesn't cap the number
	// of guesses. Tracked in a dedicated table keyed by (user, level) - not
	// the shared `users` table columns - so tripping the lockout while
	// testing one difficulty level can never affect another level's
	// independent login attempts.
