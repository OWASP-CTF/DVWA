...
if( isset( $_GET[ 'Login' ] ) ) {
	...
	// Real anti-automation: lock an account out for a while after too many
	// failed attempts in a row. A fixed sleep() alone can't stop scripted
	// brute-forcing since it just slows the same guaranteed-to-eventually-
	// succeed loop down, it doesn't cap the number of guesses.
	$max_failed_login = 3;
	$lockout_minutes  = 1;
