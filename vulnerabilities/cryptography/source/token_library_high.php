...
// CBC mode provides no integrity protection, and reusing a fixed IV for every token made it
// worse still: an attacker who can see a ciphertext can flip bits in it (e.g. to turn
// "userid:2" into "userid:1") and the decrypted plaintext changes predictably, with no way for
// the server to detect the tampering. Switched to AES-256-GCM with a random per-token IV and
// an authentication tag, matching token_library_impossible.php - any bit-flip now invalidates
// the tag and decryption fails outright.
